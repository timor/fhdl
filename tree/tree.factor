...
compiler.tree.escape-analysis compiler.tree.escape-analysis.check
...
compiler.tree.propagation.info compiler.tree.recursive
compiler.tree.tuple-unboxing formatting graphviz graphviz.notation
...
! debug tools: get information about slot literals generated by mutable lexical vars
GENERIC: slot-box ( node -- box )
M: node slot-box drop f ;
M: register-write-call slot-box in-d>> second actually-defined-by first node>> literal>> ;
M: register-read-call slot-box in-d>> first actually-defined-by first node>> literal>> ;

: reg-slot-boxes ( tree -- seq )
    V{ } clone swap
    [ compute-def-use
      [ slot-box suffix! ] each-node sift
    ] with-scope ;
